...
- **Subscription Active Priority Resolution**: Resolved a dashboard status mismatch (showing `PAST_DUE` instead of `ACTIVE`) by querying subscriptions in precedence order (`ACTIVE` -> `TRIALING` -> `PAST_DUE`) instead of strictly returning the newest by `createdAt desc`. This ensures that multiple checkout attempts that create newer pending `PAST_DUE` records do not mask the actual paid `ACTIVE` subscription.
- **Custom Domain Connection Unique Constraint Resolution**: Modified custom domain service to permanently delete (`delete`) domain records instead of soft-deleting them when removing custom domains, and automatically clean up any existing soft-deleted records when adding/creating a domain. This prevents Postgres database unique constraint violations (`P2002`) on the `hostname` field, avoiding `500 Internal Server Error` crashes when reconnecting custom domains.
